...
const ActionButton: React.FC<{icon: React.ReactNode, label: string, onClick?: () => void}> = ({ icon, label, onClick }) => (
    <div className="flex flex-col items-center space-y-2">
        <button 
            onClick={onClick}
            className="bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-full w-14 h-14 flex items-center justify-center transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-slate-900"
            aria-label={label}
        >
            {icon}
        </button>
      ...
      <main className="container mx-auto p-4 md:p-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-5xl md:text-6xl font-bold text-white tracking-tight">
            ${balance}
            <span className="text-3xl md:text-4xl text-slate-400 ml-2">{currency}</span>
          </h1>
        </div>
        ...
        <div className="max-w-md mx-auto">
            ...
            <div className="mt-6 flex justify-center">
                <button
                    ...
                    className="bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 px-6 rounded-lg shadow-lg transform hover:scale-105 transition-transform duration-300 ease-in-out flex items-center justify-center"
                    aria-label={isCardFlipped ? "Hide Card Details" : "Show Card Details"}
                >
                     <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                    </svg>
                    {isCardFlipped ? 'Hide Card Details' : 'Show Card Details'}
                </button>
            </div>
        </div>
        ...
        <div className="max-w-md mx-auto mt-10">
          <div className="grid grid-cols-4 gap-4">
            <ActionButton icon={<DepositIcon className="w-6 h-6 text-cyan-400" />} label="Deposit" onClick={() => setIsDepositModalOpen(true)} />
            <ActionButton icon={<SendIcon className="w-6 h-6 text-cyan-400" />} label="Send" onClick={() => setIsSendModalOpen(true)} />
            <ActionButton icon={<SwapIcon className="w-6 h-6 text-cyan-400" />} label="Swap" />
            <ActionButton icon={<WithdrawIcon className="w-6 h-6 text-cyan-400" />} label="Withdraw" onClick={() => setIsWithdrawModalOpen(true)} />
          </div>
        </div>
